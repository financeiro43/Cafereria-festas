...
// Helper function to format any card identifier to 16 digits consistently
...
export default function AdminDashboard({ profile, forcedTab }: { profile: UserProfile, forcedTab?: AdminTab }) {
  ...
  const handleSavePhysicalCard = async () => {
    ...
    const rawInput = physicalCardInput.trim();
    if (!rawInput) {
      toast.error('O código do cartão não pode ser vazio');
      return;
    }

    const cardCode = extractCardCode(rawInput);
    if (!cardCode) {
      toast.error('Código de cartão inválido');
      return;
    }

    try {
      toast.loading('Vinculando cartão...', { id: 'link-card' });
      ...
      if (!snap.empty) {
        const foundUserDoc = snap.docs[0];
        const foundUserData = foundUserDoc.data() as UserProfile;
        
        // A placeholder card is characterized by isPhysicalCard: true AND email containing maestro.internal or starting with card-
        const isPlaceholder = foundUserData.isPhysicalCard === true && 
          (foundUserData.email?.includes('@maestro.internal') || foundUserData.email?.startsWith('card-'));

        if (isPlaceholder) {
          // It's a pre-generated card placeholder. We can link/merge it!
          placeholderDocId = foundUserDoc.id;
          balanceToTransfer = foundUserData.balance || 0;
        } else {
          // If the card is already linked to an actual user, we cannot overwrite it
          toast.dismiss('link-card');
          toast.error(`Este cartão já está vinculado a outro usuário: ${foundUserData.name}`);
          return;
        }
      }

      // Update the target user with the card code and transfer balance
      await updateDoc(doc(db, 'users', linkingUser.uid), {
        qrCode: cardCode,
        isPhysicalCard: true,
        balance: (linkingUser.balance || 0) + balanceToTransfer,
        _backendSecret: 'FESTA_PASS_SRV_2026_SECRET'
      });

      // If we linked a placeholder card, delete the placeholder document so there is no duplicate in DB
      if (placeholderDocId) {
                            ...
                            {(!user.qrCode || user.qrCode.startsWith('PENDING-')) ? (
                              ...
                              <div className="flex items-center gap-2 py-1">
                                <div className="font-mono text-xs text-blue-600 font-bold bg-blue-55/85 px-3 py-1.5 rounded-xl border border-blue-100 flex items-center gap-2">
                                  ...
                                  <span>{formatCardNumber(user.qrCode)}</span>
                                  <button 
                        ...
                        filteredRechargeUsers.map(u => (
                          ...
                          >
                            ...
                            <div className="flex justify-between items-center mt-1">
                              <span className="text-[9px] font-mono font-bold text-blue-400">Cartão: {u.qrCode ? formatCardNumber(u.qrCode) : 'N/A'}</span>
